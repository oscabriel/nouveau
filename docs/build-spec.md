...
- **Signals, in order, first decisive wins** (sampled 2026-09-10 across the 20 seed feeds, `packages/backend/convex/extraction.ts` `classifyLot`):
  ...
  2. Title says it isn't a single roasted coffee, whatever the type says: subscription, bundle, sampler, gift set/box, k-cup/capsule/pod, cold brew/latte/RTD, bulk, add-on, hard-goods nouns (scale, grinder, kettle, mug, tee, tote…). Counter Culture files gift subscriptions under `Coffee`; Proud Mary files a Comandante grinder under `coffee-archive`.
  ...
  5. Untyped or unknown type: tags with a coffee vocabulary (`coffee`, `Coffee Type:`, `origin:`/`From:`/`Country:`, `Process:`, `Roast:`, a bare process, `Instant Craft Coffee`) → lot.
  6. Untyped, untagged: a title with coffee vocabulary (blend, espresso, decaf, instant, roast, a variety, a process, a producing country or famous region) → lot, unless it names a consumable or a book (tea, matcha, chocolate, syrup, atlas…).
...
- **Cleanup is part of the crawl.** A products.json crawl reports the ids it rejected alongside the ids it saw. `finalizeCrawl` deletes a rejected product that is still in the catalog (with its variants and drop events) instead of waiting three strikes to archive it, because it never was a lot. Exception: a rejected product someone has logged is archived, not deleted (§14.1: logs never lose their lot). This also means a tightened rule set cleans prod on the next crawl, no operator step.
