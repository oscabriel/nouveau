...
### 2026-09-19 - 2c5fefe - ADR-0010: Jev picks the line, the cutters keep the value

The page reader's label router could only read `Label: value` lines, so Regalia's colon-less spec block ("Process Washed", "Region Planadas, Tolima, Colombia"), Merit's and Coava's lead-in notes and most producers never became candidates. A live diff of four page sources and a Jev probe of whole-line options against the span candidates on 19 seed pages (ADR-0010, accepted the same day) showed the line is the right grain for locating a fact and the wrong grain for storing one: lot names verify as producers, a label without a colon stays in the value, a list fails as a whole. The reader is now two steps (`extraction.ts`: `pageElements`, `pageFactsFromPicks`): every distinct line of the page head is an option on one Choice per field, each note-shaped line gets a Noul, and the field's own cutter takes the value from the picked line before `verifyPageFacts` gates it. The 19 probe pages, reduced through the repo's reducer and paired with the recorded Jev answers, run end to end as a fixture (`convex/fixtures/probePages.json`); across them, producer goes from 7 to 10 and region from 6 to 8 with process, elevation and notes held. Two reducer defects surfaced on the way and are fixed: a popover inside Counter Culture's notes wrapper read as notes, and Onyx's 23 hover popups were 60k of its 69k characters. Earlier the same day, `c6586cc` decoded the named entities a census of all 20 seed pages found (Intelligentsia's "Chinacla&comma; La Paz") and set the Firecrawl budget to the Free plan's 10 a minute. Piece 3 of the ADR (Firecrawl `html` first, paced deferrals) is next. Suite 514.

### 2026-09-19 - 12e0a50 - ADR-0010 piece 3: Firecrawl html first, deferred reads reserve their slot

The page read now asks Firecrawl for the rendered page first (`formats: ["html"]`, cached a day, the same reducer as the shop's own page) and keeps the plain fetch as the fallback when Firecrawl answers 429 or errors, so Passenger's client-rendered block and Merit's 404 can finally be read. Every read takes a token from the deployment's Firecrawl budget, renamed `firecrawlReads` (10 a minute, the Free plan). The Decision text had a deferred read reschedule after the bucket's `retryAfter` and ask again; with 500 reads a crawl against 10 a minute that is hundreds of thousands of action runs, so a read past the budget asks the shop's own page at no credit and, failing that, reserves the next free slot with the rate limiter and reschedules itself once for that time (`pageFacts.ts`, ADR-0010 amendment). A Firecrawl 429 the shop cannot cover defers a minute, `MAX_READ_DEFERRALS` bounds both kinds, and the worker's deferred read still throws as a failure. One dependency surfaced on the way: `sentenceCandidates` split on blank lines, which only Firecrawl's markdown had, so the worker's page evidence would have gone silent on block text; it splits per line now. Not deployed yet. Suite 517.

### 2026-09-19 - bb30df1 - the Firecrawl budget holds one token; piece 3 in prod

Deployed `12fb3d9` (piece 3 plus its review follow-ups) to prod (owner consent; 17.2s), reset Passenger (85 lots) and Merit (27) with `clearFacts`, and swept both by hand: 50 reads. The first three minutes logged fifteen Firecrawl 429s. A token bucket at capacity 10 admits capacity plus rate inside any fixed minute, Firecrawl's minute is fixed, and Firecrawl counts every request against it while the component retries a 429 three times with no option to turn that off, so one read over the line burned the rest of the minute ("Consumed (req/min): 14, 18, 22, 26, 30"). The bucket now holds one token at nine a minute (`FIRECRAWL_PLAN_SCRAPES_PER_MINUTE` minus one), which keeps any sixty seconds at ten. Deployed `bb30df1` (17.1s) and swept Passenger again: 24 reads, no 429, nothing pending after four minutes. The 50 reads of the first pass all finished despite the storm, since a 429 defers a minute and comes back. Passenger after 49 rendered reads: producer on 24 (the plain-fetch pass had 19 on 85 reads), region 15, notes 34, elevation 4 (down from 21 on 85; worth a look at what Passenger's rendered spec block says). Merit on 25 reads: 16 lots with facts, producer 6, region 9, elevation 8, level with the plain pass. Suite 519.
