...
- **Log form** (`log-form.tsx`): inline under a hairline, no box. The tasting tags input beside the roaster's notes under caps labels (the picker retired 2026-09-21); RATE IT as a dot toggle that reveals a 128px native range (`accent-foreground`), the stars and the value; REVIEW as a caps label over one line of cell type on a hairline that grows with the text; SAVE LOG (UPDATE LOG when editing) in ink and CANCEL in grey as caps actions.
