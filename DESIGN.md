...
- **Footer** (`site-footer.tsx`, ADR-0012, amended 2026-09-21): one link row mirroring the header: NOUVEAU, ROASTERS, DROPS left, BACK TO THE TOP centered, ABOUT, GITHUB right; no user links. Below `md` the row stacks as three left-aligned rows: the left links, BACK TO THE TOP, then ABOUT and GITHUB, no flex-wrap. Under the hairline the flower (`02-five-stamina.webp`, linking the Coffea arabica article, fading to 0.8 on hover) and NOUVEAU.COFFEE in the landing wordmark's Garamond italic, right-aligned from `md`, centered and full-width below. The webp is square with the drawing at 448x361 of the 540x540 canvas (symmetric margins), so the element rides at the wordmark's own font-size from `md` (10.5vw below is not matched; the flower stays 128px there), and the visible drawing then sits at the caps' height, bottoms flush with the text's baseline. The other five details and their Caveat captions are gone. Mounted once in the root layout.
