...
- **Plate assets** (`apps/web/public/`, ADR-0012): two webp exports, cream fringe fixed. `coffea-arabica.webp` is the trimmed branch (1600×1671 with a 16px transparent margin, cut from a 3260px PNG master kept outside the repo; the ImageMagick line lives in `lib/branch.ts`) that the wordmark carries and the about page shows as the plate; there is no standalone hero image anymore. `coffea-arabica-flower.webp` is the five-stamen flower the footer shows. The other five detail exports were removed on 2026-09-21 and live in git history if a page ever needs them.
