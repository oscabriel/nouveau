...
Each crawl commits in one transaction: raw capture, catalog upserts, drop-event diffing, and rescheduling. A roaster's first crawl fills its catalog and fires nothing; from the second on, a new lot, a back-in-stock or a price drop becomes a drop event. Events feed the landing index and `/drops`, and every alert-worthy one fans out to the roaster's unmuted watchers inside the transaction that emitted it. One notifications-ledger row per (user, event) doubles as the one-email-per-event dedupe guard; sends go through the workpool, and AgentMail delivers from one shared inbox to each watcher's own email, replies threaded in that inbox.

Firecrawl covers what feeds don't carry. Shops with no feed are read page by page in Firecrawl's product format, gated by change tracking on the collection page, and Firecrawl also takes over any feed URL that fails. Tasting notes usually live in a Shopify metafield the feed never carries, so after each crawl a sweep reads the product pages of lots still missing facts, Firecrawl's rendered HTML first and the shop's own page as fallback, all under a token-bucket budget; a viewer reaching a thin lot first triggers the read on view. Every read asks TypeSafe's Jev to pick the line holding each fact, a cutter takes the value from that line, and verifiers gate what gets stored. Jev also shadows the lot classifier's ambiguous tail, recording without acting.
