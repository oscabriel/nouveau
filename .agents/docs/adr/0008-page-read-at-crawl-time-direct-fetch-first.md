...
- `FIRECRAWL_FALLBACK_PER_MINUTE` is 10, the Free plan's /scrape limit the prod key is on (2 concurrent, counted per team, 429 past either). It had been 60, six times the limit, so the bucket admitted reads Firecrawl then refused; those reads took the 429 path (stamp kept, not counted) instead of the deferral path. The crawler's own scrapes share the 10 without passing through the bucket. ADR-0010 (proposed) makes Firecrawl the first source and revisits the pacing.
- Checked on prod the next morning (26b5a6a deployed 2026-09-19 02:24 UTC): Sey's seven purchasable lots were read within the hour and no sold-out Sey lot was read after the deploy. The backfill of every roaster finished 45 minutes after the deploy (729 reads, 589 sweeps, no failures) and the hourly sweeps have scheduled nothing since. Five purchasable lots (four Merit, one Ruby) sit in the 24 hour retry window because their pre-deploy reads hit the Firecrawl per-minute limit; the deferral path keeps the stamp without counting a read, as designed.
