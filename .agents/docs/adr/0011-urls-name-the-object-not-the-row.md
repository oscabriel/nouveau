...
**2026-09-20 (design-pass review, section 1 and S4). The new lot takes the year; the first lot keeps its handle.** The year-suffix rule as written moved the archived lot to `ethiopia-guji-2024` and handed the bare handle to the new lot, so every alert email, log link and saved link to the old lot silently landed on the new one. The owner reversed it: the first lot with a handle keeps it for good, and each later lot arriving with the same handle in the same shop takes the year it was first seen (`ethiopia-guji-2025`), then a number when that year is taken too (`ethiopia-guji-2025-2`). In practice no lot carries a year today; the suffixes start appearing as coffees return next season. The rule now applies whether the earlier lot is archived or still current, so two feed items sharing a handle (plausible for WooCommerce and product-page shops, where the handle is the last URL segment) no longer leave two current lots under one handle and `lots.get`'s `.unique()` cannot throw. `crawlSources.ts:claimLotHandle` decides the handle at insert through the roaster-plus-handle index; a stored lot's handle is never rewritten.
