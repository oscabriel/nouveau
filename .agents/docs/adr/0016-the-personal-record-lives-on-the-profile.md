...
**2026-09-21 (owner's work-plan-2 brief). Settings are one page with tabs; the account tab holds the weight unit; roaster notes join the pills.** `/settings` renders a title and three caps tabs, ACCOUNT, ALERTS, APPEARANCE, with the chosen tab's content under them, and lands on Account. Account gains a Weights choice, Grams or Ounces, stored as `users.weightUnit` (`metric` or `imperial`, metric when unset). Grams stay the stored unit on every product; the choice only changes how a weight renders, and imperial snaps to the bag sizes roasters sell (340 g reads 12 oz, 2268 g reads 5 lb). The batch 8 line "roaster notes stay plain text" is withdrawn: the roaster's descriptors render as the same family-colored pills in the catalog table and on log cards, each linking to `/drops?family=`, so every lot note on the site reads one way.
