...
**2026-09-21 (owner, same list). A run stores what it reads.** "A run writes no facts unless asked" is reversed: `commit` leaves `start` and the picker, and `runLot` always calls `internal.pageFacts.store` after the read, exactly as the scrape does. A run over a roaster is now a hand-started sweep of its ten thinnest lots, with traces. The reason for the toggle was caution about a demo overwriting production facts; the read is the same code path the sweep runs unattended every crawl, so the caution guarded nothing. The run document's `commit` field stays optional until the rows from before are pruned, then goes.
