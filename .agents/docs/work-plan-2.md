...
| ---- | ----- | ----------- |
| 2026-09-21 | review pass, deploy | Web UI review pass committed as four commits (`ea6f2c2..7d73bc0`) and deployed to prod `artful-chameleon-402` via `bun run deploy`; every earlier row in work-plan.md marked "Not yet deployed to prod" is live as of this deploy. Sign out works again (`DropdownMenuItem` is Base UI and takes `onClick`, `onSelect` was silently dropped). Branch recut at 1600×1671 (433 KB) from the 3260px master, which now lives outside the repo; the footer flower is `public/coffea-arabica-flower.webp` and `src/assets/` is gone. One page shell (`components/page.tsx`, `pane.tsx`, `mute-toggle.tsx`), shared class constants in `lib/ui.ts`, redirect routes in `beforeLoad`, a root `notFoundComponent`, static `head` titles, `describeMutationError` in every catch. `tsc` clean, Ultracite clean, 7 web tests. Left: `DropTypeTabs` tab roles without a tablist keyboard model, dynamic titles for roaster, lot and profile pages, the lot page's own 5-second interval, `og.png` still to be dropped into `public/`. |
