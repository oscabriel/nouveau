...
- [x] **Jev request structuring.** After submit, one parallel batch of Choice / Noul judgments turning the free text into typed filters (budget, bag size, origin, process, flavour direction) that the search tool takes as typed args. Reuses `askJev` and the pinned `jev-1.13.0`.
